"""Fills the 27 (of 1,464) Santo Domingo buildings missing
`roof_material` (all in ensanche_quisquella, see
scripts/exposure/README.md) using the `roof_material` package's
reference-based (Hungarian algorithm) cluster mapping -- santo_domingo's
other 1,437 buildings already have real surveyed roof_material, used as
ground truth to resolve the mapping automatically (no manual visual
judgment call, unlike apply_lomas_centinela_roof_material.py).
...
Source run: roof_material/examples/SantoDomingo/santo_domingo_example.py,
writing roof_material/examples/SantoDomingo/output/buildings_roof_material.gpkg.
That output's own `id`/`building_id` columns are a renumbered sequential
integer; the int-id<->real-id mapping is recovered from
roof_material/examples/SantoDomingo/santo_domingo_footprints_intid.gpkg.

Usage (from backend/, after the roof_material example run above):
uv run python scripts/exposure/apply_santo_domingo_roof_material.py
"""
...
ROOF_MATERIAL_DIR = Path(__file__).resolve().parents[4] / "roof_material" / "examples" / "SantoDomingo"
ROOF_MATERIAL_RESULTS_GPKG = ROOF_MATERIAL_DIR / "output" / "buildings_roof_material.gpkg"
ROOF_MATERIAL_INTID_GPKG = ROOF_MATERIAL_DIR / "santo_domingo_footprints_intid.gpkg"


def apply_santo_domingo_roof_material() -> None:
    ...
    if not ROOF_MATERIAL_RESULTS_GPKG.exists() or not ROOF_MATERIAL_INTID_GPKG.exists():
        raise RuntimeError(
            ...
            "roof_material/examples/SantoDomingo/santo_domingo_example.py first"
        )
...
if __name__ == "__main__":
    apply_santo_domingo_roof_material()
